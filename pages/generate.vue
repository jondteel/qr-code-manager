  ...
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="flex justify-between items-center h-16">
          <div class="flex items-center gap-8">
            ...
            <div class="hidden md:flex items-center gap-x-8 whitespace-nowrap">
              ...
              <NuxtLink to="/generate" class="text-blue-600 font-medium">Generate QR</NuxtLink>
          ...
          <div class="flex items-center gap-4">
            ...
            <button class="text-gray-600 hover:text-gray-900 text-sm font-medium">Sign Out</button>
          </div>
        </div>
      </div>
    </nav>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Customize size, colors, margin, and error correction. Live preview updates as you type.</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div class="space-y-6">
            <!-- Content -->
            <div>
              <label class="block text-sm font-medium text-gray-900 mb-1">QR Content</label>
              <textarea
                v-model="content"
                rows="4"
              ...
              <div class="flex items-center justify-between mt-2">
                <p class="text-xs text-gray-500">Tip: URLs are most common (e.g., https://example.com)</p>
                <p class="text-xs text-gray-500">{{ content.length }} chars</p>
              </div>
            </div>

            <!-- Analytics toggle (UI only for now) -->
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="text-sm font-medium text-gray-900">Enable Analytics (coming next)</p>
                  <p class="text-xs text-gray-600 mt-1">
                    Later we’ll generate a tracked short URL (redirect) so scans can be counted.
                  </p>
                </div>

                <button
                  type="button"
                  @click="enableAnalytics = !enableAnalytics"
                  class="relative inline-flex h-6 w-11 items-center rounded-full transition"
                  :class="enableAnalytics ? 'bg-blue-600' : 'bg-gray-300'"
                  aria-label="Toggle analytics"
                >
                  <span
                    class="inline-block h-5 w-5 transform rounded-full bg-white transition"
                    :class="enableAnalytics ? 'translate-x-5' : 'translate-x-1'"
                  />
                </button>
              </div>

              <div v-if="enableAnalytics" class="mt-4">
                <label class="block text-sm font-medium text-gray-900 mb-1">Name (for your dashboard)</label>
                <input
                  v-model="label"
                  type="text"
                  class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                ...
                <p class="text-xs text-gray-600 mt-2">
                  For now this just labels the QR. Later we’ll store it + create a tracked short link.
                </p>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div class="flex items-center gap-3">
                  <input v-model="fgColor" type="color" class="h-10 w-14 p-1 rounded border border-gray-300 bg-white" />
                  <input
                    v-model="fgColor"
                    type="text"
              ...
              <div>
                ...
                <div class="flex items-center gap-3">
                  <input v-model="bgColor" type="color" class="h-10 w-14 p-1 rounded border border-gray-300 bg-white" />
                  <input
                    v-model="bgColor"
                    type="text"
                    class="flex-1 rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div class="flex flex-col sm:flex-row gap-3">
              <button
                ...
                :class="(!canGenerate || isGenerating) ? 'opacity-60 cursor-not-allowed' : ''"
                @click="downloadPng"
              >
        ...
        <section class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-6 flex items-center justify-center">
              <div class="flex flex-col items-center gap-3">
                ...
                <p class="text-xs text-gray-500">
                  {{ canGenerate ? 'Preview updates automatically.' : 'Enter some content to generate a QR.' }}
                </p>
              </div>
            </div>
          </ClientOnly>
          ...
          <div class="mt-6 rounded-lg border border-gray-200 p-4">
            <h3 class="text-sm font-semibold text-gray-900 mb-2">What’s next (analytics)</h3>
            <ul class="text-sm text-gray-600 space-y-1 list-disc pl-5">
              ...
              <li>Store QR records in Supabase (user_id, label, destination, created_at)</li>
...
const enableAnalytics = ref<boolean>(false)
...
async function downloadPng() {
  ...
  try {
    ...
    // Ensure the canvas is rendered at current settings before download
    await renderQr()

    const dataUrl = canvas.toDataURL('image/png')

    const safeLabel = (enableAnalytics.value && label.value.trim().length > 0)
      ? label.value.trim().replace(/[^\w\-]+/g, '_').slice(0, 60)
      : 'qr_code'
    ...
    errorMessage.value = e?.message ?? 'Failed to download PNG.'
  } finally {
